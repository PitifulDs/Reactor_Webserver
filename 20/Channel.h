...
private:
    ...
    std::function<void()> readcallback_;    // fd_读事件的回调函数 ，如果是acceptchannel，则回调Acceptor::newconnection()
    std::function<void()> closecallback_;    // 关闭fd_的回调函数,将回调Connection::closecallback()
    std::function<void()> errorcallback_;    // fd_错误事件的回调函数,将回调Connection::errorcallback()
...
public:
    ...
    void setclosecallback(std::function<void()> fn);    // 设置fd_关闭事件回调函数
    void seterrorcallback(std::function<void()> fn);    // 设置fd_错误事件回调函数    
};